...
% "Introduction" exists in each chapter and gives short intro to chapter + what can be expected in chapter

% OLD INTRO: https://docs.google.com/document/d/1oJssCGQf0aSDCa4xxY4UMJMsaGYdrbOSlzk6mLIS4no/edit?usp=sharing
...
Because of this, \glspl{bci} are also referred to as \glspl{bmi} and can be seen as a special type of \glspl{hmi}.
...
Chapter \ref{ch:biomedical_signals} and \ref{ch:processing_signals} focus on more technical aspects of the \gls{bci} field.
...
These first chapters aim to introduce these concepts in an easy-to-understand manner for a typical computer scientist student with some \gls{ai} background.
...
% GAINING POPULARITY
% ---------------------------------------------- 
% discussing the rise in popularity of both medical and commercial BCI
...
However, an invasive approach introduces added health risks and even more ethical challenges \citep{neuralink_ethics, neuralink_ethics2}.
...
This headset relies on measuring Theta waves in the brain, which are lower frequency waves that suggest a user is meditating. % TODO: link to a table of freqs
...
This is not surprising as the promises of what a \gls{bci} can do are near endless and initial trials often offer promising results but going to a final product has been proven to be incredibly hard for these commercial companies.
...
% TODO add figure met kleurtjes van components zodat naar kleurtjes kan reffen en die refs naar kleurtjes in tekst steken
...
Due to Moore's law, \glspl{cpu} and other computational hardware have also seen massive improvements in computational power.
...
These M series processors have a neural engine that is stated to accelerate the time needed for \gls{ml} tasks\footnote{\url{https://www.apple.com/newsroom/2021/10/introducing-m1-pro-and-m1-max-the-most-powerful-chips-apple-has-ever-built/}}.
...
To summarize, early attempts at making \glspl{bci} more portable and affordable focused on splitting the brain signal measuring equipment from the data processing equipment \citep{early_bci_drowsiness, early_bci_multimedia}.
...
\section{Medical use cases for BCIs}
...
\label{subsec:bci_common_use_casesnovel_interaction}
...
\label{subsec:bci_common_use_casesprostheses}
...
% Mss die porn detection dinges uit nerdland podcast?
TODO
...
\subsection{An issue of standardized testing and optimistic results}
...
\subsection{BCI research is highly interdisciplinary}
...
\label{sec:bci_opportunities_obstacles_ours}
