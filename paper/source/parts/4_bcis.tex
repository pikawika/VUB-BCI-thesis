...
% TODO: start hier, niet te groot

% brainmapping
There are still a lot of unknowns on the complete structure and working of the brain, with no exact mapping of the brain in existence \citep{brainmapping}.
As such, there are still a lot of unknowns with interpreting brain signals.
This makes the development of great feature extraction methods or \gls{dl} models for brain signals challanging.
It is known that the brain works in a a dynamic, nonstationary and non-linear way \citep{bci_applications}.
Non linearity is a common challenge in many research fields and many \gls{ml} and \gls{dl} models exist that can work with such data.
However, the dynamic and nonstationary nature of brain signals are less common and form an extra challenge.
This nonstationary arise mostly from what could be seen as internal noise.
Internal noise can be seen as brain signals that are present next to the brain signal of interest.
Even though the envisioned \gls{mi} task may be identical, the presence of brain activity for breathing, moving, listening to sounds and much more will continuously vary.
The brain's capability of learning 
Neuroplasticity, which was already discussed in Section \ref{subsec:bci_common_use_cases_medical_phenomena}, can also be seen as a source of internal noise.
External noise includes common artifacts such as electrical interference and will be discussed in more detail in Section \ref{subsec:biomedical_signals_measuring_artefacts}.
