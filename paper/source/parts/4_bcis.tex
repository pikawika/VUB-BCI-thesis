...
% NOTE: \subsubsection{Detailed literature on MI pipelines} all spell checked etc, gewoon nog 1x lezen
...
Many articles still fail to provide the source code of their implemented models and lack the required detail to fully recreate the models \citep{bci_review_arnau}.
This makes articles paying special attention to describing their model and discussing their design decisions stand out even more.
Not only do such articles make creating the model providing libraries discussed in Section \ref{subsec:bci_gaining_popularity_improved_data_processing} possible, but they also give a good insight into what components are necessary for a \gls{bci} system.

One article that focuses on this is the one by \citet{cheap_bci_feasibility} discussing the feasibility of a complete low-cost consumer-grade \gls{bci} system.
...
\Citet{cheap_bci_feasibility} opting for wet electrodes is slightly odd and makes the proposed system less compliant with Wolpaw's definition of a perfect \gls{bci}.
...
The room was free of external stimuli as it was separated from the experiment organisers and the participant was \textit{left alone} for the entire trial.
...
In one trial the electromagnetic noise amplitude, which is 50Hz or 60Hz depending on location, was four times higher than the meaningful EEG data.
Other types of noise were also present, so whilst it was by no means a perfect lab-like environment, calling it a real-world test is stretching it.
...
Part of the reason medical-grade hardware is about 5 to 10 times as expensive for a similar experiment is due to the medical-grade hardware requires certifications, which also guarantee some form of quality and reliability. 
...
\Citet{cheap_bci_feasibility} achieved an average accuracy between 70\% and 85\%, scaling with the complexity of the used \gls{ml} approach.
...
\subsubsection{Detailed literature on complex classification models}
\label{subsubsec:bci_opportunities_obstacles_motivating_examples_mi_models}

% titel aangepast dus aanpassingen heel likely
% hier zeker EEGNet, ShallowConvNet en DeepConvNet mee linken en die library met hun implementie
% ook een RNN approach zoeken die LTSTM gebruikt zodat je die kan reffen naar ons werk
...
% merge with onderstaand voor "few classifications, many actions"
% dit dus copy paste
...
% dit oude content van dit puntje
