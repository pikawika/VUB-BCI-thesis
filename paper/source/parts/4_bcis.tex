...
% START HERE; TODO
% Done untill but not motivatin examples
...
However, there are some specific works that have motivated the creation of this master's thesis which will be discussed in detail here.
...
% NOTE: this section is copy paste dus aanpassingen nodig
...
% NOTE: this section is copy paste dus aanpassingen nodig
% TODO: hier was je bezig

Section \ref{subsec:bci_common_use_cases_bcis_replace_eye_tracking} discussed P300 based \gls{bci} systems.
Whilst the P300 signal is one that is relatively easy to detect and evoke, the discussed visual stimuli used has many limitating factor.
Perhaps most limiting is that the user can not evoke the signal without reuqiring external stimuli.
The public image of a person sitting behind a flashing screen is also not appealing to users and isn't in line with Wolpaw's definition of a perfect \gls{bci}.

Detectable signals that can be evoked voluntary by the user are more interesting for most \gls{bci} applications, especially those that focus on providing \gls{bc}.
Likewise, the process the user should go through to evoke this detectable signal should be one that doesn't require physical movement to both be applicable for users suffering from limited muscle control and be in line with Wolpaw's definition of a perfect \gls{bci}.
\Glsfirst{mi} is such a cognitive process that is often used for this purpose.
