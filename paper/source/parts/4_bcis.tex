...
This muscular activity can then easily be measured with the \gls{emg} modality, which is in general of higher signal quality than \gls{eeg}.
...
Section \ref{subsec:bci_common_use_cases_improving_existing_system} discusses more of these hybrid systems outside the scope of prosthesis and exoskeleton control.
...
% START HERE; TODO
% kijk naar bci_applications voor insperatie en maak die figuur met boom voor de summary

% TODO: uitleggen dat er vaak ook non brain signal alternatieven bestaan

% TODO: van arnau: meest common robot arm control > drone control (a drone can be the well-known quadcopter (Redrovan and Kim 2018), but this can also refer to ground vehicles (Lu et al 2020) and robots (Aznan et al 2019) that can move according to the operator’s commands.) > bci spellers maar ook andere dingen zoals smart home control en virtual reality control en meer
...
The previous sections discussed use-cases of \glspl{bci} in the medical world and as control systems for prostheses and exoskeletons, where they could be seen as a standalone system.
However, \glspl{bci} are also used as an extension to existing systems, both for medical and commercial applications.
This can even happen in non-trivial domains.
...
Some examples of hybrid systems which combine multiple \gls{biosignal} measuring modalities is also discussed.
Section \ref{subsec:bci_common_use_cases_commercial_and_others} will discuss some commercial applications where \glspl{bci} are used as an extension to existing systems.
...
\subsubsection{Hybrid BCIs which rely on brain-signals among other biosignals}
\label{subsubsec:bci_common_use_cases_improving_existing_system_hybrid_bci}

% all discussed bij exoskeleton enzo als zijnde ook ogen blabla includen
% bci_common_use_cases_prosthesis_exoskeleton
TODO
...
% soort van control systems maar als replacement van iets dat ge normaal al kon
...
\subsection{More commercial and other use cases}
\label{subsec:bci_common_use_cases_commercial_and_others}
...
\subsection{Summary on the use cases of BCIs}
