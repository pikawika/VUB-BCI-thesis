...
A well-known Professor in this field, Jonathan R. Wolpaw, was part of the group that provided the following first formal definition for \glspl{bci}:
...
Considering Wolpaw's many contributions to the field, his opinion on what defines a \textit{perfect medical \gls{bci}} is often strived for \citep{bci_rehabilitation, bci_in_medicine, first_bci_meeting, bci_book}.
Whilst being focused on medical applications, these properties would also be beneficial for commercial \gls{bci} systems.
For these reasons, this first chapter relates many of the discussed topics to the definition below quite a lot.
It is noted that the term communication in these definitions simply depicts the exchange of information rather than specific human communication such as speech.
...
This first chapter aims to provide all the required background information needed for understanding \glspl{bci} on an intuitive level.
It is written in such a way that it is accessible to new people in the field that are otherwise confronted with the steep learning curve related to the \gls{bci} field.
For those that already have experience in the field or are looking for a more technical discussion of the field are invited to read diagonally through this first chapter, focusing primarly on the summaries provided in Section \ref{subsec:bci_gaining_popularity_summary} and \ref{subsec:bci_common_use_summary}, Section \ref{sec:bci_opportunities_obstacles} in its entirety and the chapter conclusions under Section \ref{sec:bci_concolusion_and_proposing_ours}.

For those interested in this chapter in its entirety, this chapter further introduces the main rationale behind \glspl{bci} research by discussing the rise in popularity of both medical and commercial \glspl{bci}, some practical examples of \gls{bci} systems that have been developed and some of the opportunities and obstacles in the field. The chapter ends with a note on some of the ethical challenges for \gls{bci} systems and a discussion of the proposed system for this thesis.
...
\begin{figure}[ht]
    ...
    \captionsetup{justification=centering}
    \caption{The anatomy of the human head, specifically of the cerebrospinal system. Non-invasive \gls{eeg} measuring equipment is placed on the scalp, causing signals from the brain to be blocked by the skull and \glsfirst{csf} among other structures. Free to use figure by \citet{figure_blausen}, CC BY 3.0, via Wikimedia Commons.}
...
In essence, these applications rely on separating the data acquisition process and data processing into two standalone systems connected over \glsfirst{bt}.
...
This headset could then be connected over \glsfirst{bt} to the visitor's phone running an app for the museum tour.
...
Another possible route the author of this paper sees is the use of cloud computing and fast 5G connections to also create a more simple user experience that doesn't require \glsfirst{bt} tethering to a close-by processing unit.
...
This process of thinking of movement but not doing the movement is known as \glsfirst{mi} and further discussed in Section \ref{subsec:biomedical_signals_working_with_eeg_inducing_methods}.
...
Especially the possibility of regaining even the simplest form of mobility through some very basic controls derived from \glsfirst{mi} is what motivated the author of this master thesis to study the field.
...
Such a general model can also be used as a base model for calibration, a process based on the idea of \glsfirst{tl} further discussed in Section \ref{subsec:online_bci_system_different_common_steps_calibration}.
...
Locks can be picked, databases can be breached, \glsfirst{bt} connections can be spoofed and even the most secure encryption algorithms are prone to hacking \citep{quantum_hacking}.
\Citet{p300_hacking} have shown that an \gls{eeg} based \gls{bci} working with P300 signals can be hacked by generating fake P300 waves from an external device.
...
% Wheelchair example from review article arnau?
