...
Data acquisition relies heavily on neuroscience for the working of the brain to know what signals can be extracted and where they originate from.
To extract those signals, engineers should develop hardware capable of measuring the brain signals.
The data classification is mainly a computer scientist task, although knowledge from neuroscience can be very helpful in this step combined with an insight into the flaws of the hardware.
The connection to an external device can relate to a wide variety of sciences once again.
For example, the previously discussed robotic arm example shown in Figure \ref{fig:example_arm_control} requires computer vision knowledge for grasp detection, engineering knowledge to make the robot arm and general computer science knowledge to create a working link between classification labels and action controls.
The user study can then require an ethics expert, people capable of studying psychological impact as well as all expertise needed for all previous parts.

With most current research still trying to propose a \gls{bci} system that is as complete as possible, the proposed system is only outstanding in one aspect.
...
However, the data acquisition, classification algorithms used and the user interface proposed could benefit from improvements to make the complete \gls{bci} system viable in the real world.
